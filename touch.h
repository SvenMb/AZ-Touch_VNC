...
int16_t touch_map_x1 = 3800;
int16_t touch_map_x2 = 500;
int16_t touch_map_y1 = 3800;
int16_t touch_map_y2 = 500;
...
bool touch_touched()
{
  ...
  if (ts.touched())
  {
    ...
    if (touch_swap_xy)
    {
      touch_last_x = map(touch_raw_y, touch_map_x1, touch_map_x2, 0, touch_max_x);
      touch_last_y = map(touch_raw_x, touch_map_y1, touch_map_y2, 0, touch_max_y);
      // Serial.printf("touch_last_x: %d, touch_last_y: %d SWAPPED!!!\n", touch_last_x, touch_last_y);
      // Serial.printf("touch_max_x: %d, touch_max_y: %d SWAPPED!!!\n",touch_max_x,touch_max_y);
    }
    else
    {
      touch_last_x = map(touch_raw_x, touch_map_x1, touch_map_x2, 0, touch_max_x);
      touch_last_y = map(touch_raw_y, touch_map_y1, touch_map_y2, 0, touch_max_y);
      // Serial.printf("touch_last_x: %d, touch_last_y: %d\n", touch_last_x, touch_last_y);
      // Serial.printf("touch_max_x: %d, touch_last_y: %d\n",touch_max_x,touch_max_y);
    }
    return true;
  }
