...
/* WiFi settings */
const char *SSID_NAME = "WLAN";
const char *SSID_PASSWORD = "WLAN_KEY";

const char *VNC_IP = "192.168.4.128";
const uint16_t VNC_PORT = 5900;
const char *VNC_PASSWORD = "12345678";
...
void setup(void)
{
  ...
  // Serial.printf("gfx->width: %d, gfx->height: %d\n",gfx->width(), gfx->height());
  ...
  gfx->begin();
  touch_init(gfx->width(), gfx->height());
  // Serial.printf("gfx->width: %d, gfx->height: %d After gfx->begin\n",gfx->width(), gfx->height());
  gfx->fillScreen(BLACK);
